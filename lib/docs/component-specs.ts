...
export const COMPONENT_SPECS: ComponentSpec[] = [
...

  {
    ...
    name: "Toast",
    description:
      "Transient notification displayed via a global provider system. Stacks and collapses/expands based on input modality. Always use the useToast() hook, never render Toast JSX directly.",
    anatomy: [
      { name: "Toast container", description: "top on compact, bottom-right on regular+" },
    ...
    props: [
      ...
      { name: "variant", type: '"default" | "success" | "warning" | "destructive"', default: '"default"', description: "Controls icon + color; warning/destructive sets role='alert'" },
      { name: "action", type: "{ label: string; onClick: () => void }", default: "—", description: "Single optional action" },
      { name: "duration", type: "number (ms)", default: "4000", description: "Auto-dismiss delay; undefined = persistent" },
    ],
    states: [
      { state: "Collapsed stack", description: "Layered peek (scale -0.05, translateY -8px per index)" },
      { state: "Expanded stack", description: "Triggered by hover (fine) or tap (coarse); full list at scale 1" },
      { state: "Enter", description: "Slide+fade from appearance direction" },
      { state: "Exit", description: "Auto (duration elapsed), swipe-dismiss (touch), or X button" },
    ],
    doDont: [
      {
        do: "Use variant='warning' or 'destructive' for urgent messages — they set role='alert' for screen readers",
        dont: "Place <Toast> JSX directly in component trees — always use useToast() hook through global <ToastProvider>",
      },
    ],
    tokens: [
