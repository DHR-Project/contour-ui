...
export function DocsCodeBlock({ children, lang = "text", copyable = false }: DocsCodeBlockProps) {
        ...
        >
          <AnimatePresence mode="wait" initial={false}>
            <motion.span
              ...
              initial={reducedMotion ? undefined : { opacity: 0, scale: 0.5, rotate: -30 }}
              animate={{ opacity: 1, scale: 1, rotate: 0 }}
              exit={reducedMotion ? undefined : { opacity: 0, scale: 0.5, rotate: 30 }}
              transition={reducedMotion ? { duration: 0 } : springs.bouncy}
              className="flex"
            >
              <Icon name={copied ? "check" : "copy"} size="sm" color={copied ? "success" : undefined} />
            </motion.span>
          </AnimatePresence>
        </button>
      )}
