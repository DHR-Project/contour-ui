 ...
 * reveal, SS4.4). Max 3 direct actions -- the 4th+ collapse into "More"
 * (SS4.1). Destructive action is last in the array so full-swipe-commit
 * (mail-app style) triggers it, not a lighter action.
 */
...
export const OverflowActions: Story = () => (
  <List>
    <ListItem
      key="1"
      title="4+ actions collapse into More"
      trailingActions={[
  ...
  return (
    <div className="w-80">
      <button
        ...
        onClick={() => setItems((prev) => [...prev, { id: nextId++, title: `Item ${nextId}` }])}
      >
        Add item
      </button>
      <List>
            ...
            trailingActions={[
              {
                ...
                label: "Delete",
                color: "destructive",
                onAction: () => setItems((prev) => prev.filter((i) => i.id !== item.id)),
              },
            ]}
          />
        ))}
