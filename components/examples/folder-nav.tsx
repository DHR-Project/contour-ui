    ...
    <Sidebar
      ...
      header={
        <VStack gap="1" className="px-(--space-5) pt-(--space-6) pb-(--space-3)">
          <Text textStyle="large-title" weight="semibold">
            Notes
          </Text>
          <Text textStyle="footnote" color="secondary">Take your note here</Text>
        </VStack>
      }
      items={items}
      value={activeFolder}
      ...
      footer={!isCompact ? <AccountFooter /> : undefined}
    />
  );
}
    ...
    <div className="border-t border-separator px-(--space-3) py-(--space-3)">
      <HStack align="center" gap="2" container={false}>
        <Dropdown
          trigger={
            <button
              ...
              className="relative shrink-0 rounded-full outline-none focus-visible:outline-solid focus-visible:[outline-width:var(--focus-ring-width)] focus-visible:outline-offset-(--focus-ring-offset) focus-visible:outline-[rgb(var(--focus-ring-color))]"
            >
              <Avatar name="Jordan Lee" size="sm" />
              <Badge
                dot
                className="absolute -bottom-0.5 -right-0.5 ring-2 ring-bg-primary"
              />
            </button>
          }
          side="top"
          ...
          items={[
            { type: "label", text: "Jordan Lee" },
            {
              ...
              icon: "user",
              label: "View Profile",
              onSelect: () => toast({ title: "Profile (demo only)" }),
            },
            {
              type: "action",
              icon: "settings",
        ...
        <VStack gap="0" container={false} className="min-w-0 flex-1">
          ...
          <Tooltip
            content={syncing ? "Syncing changes…" : "All changes synced"}
          >
            <span
              tabIndex={0}
              className="inline-flex w-fit items-center gap-1 outline-none"
            >
              {syncing ? (
