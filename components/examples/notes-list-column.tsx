...
import { formatRelativeTime, notePreview, MOVABLE_FOLDERS } from "@/lib/examples/notes-data";
...
export function NotesListColumn() {
  ...
  const searchResults = useMemo<SearchFieldResult[] | undefined>(() => {
    ...
    return notes
      .filter((n) => !n.deleted && (n.title.toLowerCase().includes(q) || n.body.toLowerCase().includes(q)))
      .slice(0, 8)
      .map((n) => ({
        id: n.id,
  ...
  function handleNewNote() {
    const folderId: NoteFolderId = activeFolder === "personal" || activeFolder === "work" ? activeFolder : "personal";
  ...
  function trailingActionsFor(note: Note): SwipeAction[] {
    if (inTrash) {
      return [
        { icon: "circle-check", label: "Restore", color: "default", onAction: () => restoreNote(note.id) },
        { icon: "trash", label: "Delete", color: "destructive", onAction: () => setConfirmDeleteId(note.id) },
      ];
    }
    const base: SwipeAction[] = [
      { icon: "star", label: note.pinned ? "Unpin" : "Pin", color: "default", onAction: () => togglePin(note.id) },
      { icon: "share", label: "Share", color: "tint", onAction: () => handleShare(note) },
    ];
    ...
    if (note.id === "n2") {
      base.push({ icon: "copy", label: "Duplicate", color: "default", onAction: () => duplicateNote(note.id) });
    }
    base.push({ icon: "trash", label: "Delete", color: "destructive", onAction: () => softDeleteNote(note.id) });
    return base;
  }

  function contextMenuItemsFor(note: Note): DropdownItemDef[] {
    return [
      { type: "action", icon: "star", label: note.pinned ? "Unpin" : "Pin", onSelect: () => togglePin(note.id) },
      { type: "action", icon: "copy", label: "Duplicate", onSelect: () => duplicateNote(note.id) },
      {
        type: "submenu",
        icon: "layers",
        label: "Move to",
      ...
      { type: "separator" },
      {
        ...
        role: "destructive",
        onSelect: () => (inTrash ? setConfirmDeleteId(note.id) : softDeleteNote(note.id)),
      },
    ];
  }
  ...
  return (
    <>
      <div className={cn("flex flex-col", !isCompact && "h-full")}>
        <div className={cn(!isCompact && "flex-1 min-h-0 overflow-y-auto scroll-mask-y")}>
          <NavBar
            ...
            largeTitleMode
            trailingActions={[
              { icon: "sliders-horizontal", label: "View Options", onClick: () => setViewSheetOpen(true) },
          ...
          {items.length === 0 ? (
            <Container variant="content">
              <VStack gap="3" align="center" className="py-(--space-10) text-center">
                <Icon name={folder.icon} size="lg" className="text-label-tertiary" />
            ...
            <List style="plain">
              {items.map((note) => (
                <ListItem
                  ...
                  trailingText={formatRelativeTime(note.updatedAt)}
            ...
            <Grid columns={{ compact: 1, regular: 2 }} gap="3" className="p-(--space-4)">
              {items.map((note) => (
                <ContextMenu key={note.id} items={contextMenuItemsFor(note)}>
                  ...
                  >
                    <VStack gap="1" container={false}>
                      <HStack justify="between" align="start" gap="2" container={false}>
                        <Text textStyle="subheadline" weight="semibold" truncate className="min-w-0 flex-1">
                          {note.title || "New Note"}
                        </Text>
                        {note.pinned && <Icon name="star" size="xs" className="mt-0.5 shrink-0 text-label-tertiary" />}
                      </HStack>
                      ...
                      <Text textStyle="caption-2" color="tertiary" className="mt-(--space-1)">
          ...
          {isCompact && <div aria-hidden style={{ height: "calc(64px + var(--safe-area-bottom))" }} />}
        </div>
      </div>
      ...
      <Alert
        ...
        actions={[
          { label: "Cancel", role: "cancel", onClick: () => setConfirmDeleteId(null) },
          {
            label: "Delete",
            role: "destructive",
            emphasized: true,
